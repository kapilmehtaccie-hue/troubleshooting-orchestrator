...
Kapil Mehta, Prashant Sumanprasad Bhadoria, Jaypal Baviskar. An AI Orchestrator Model for Troubleshooting Skills Training in Network Engineering. SIGCOMM Education Workshop 2026. https://a4ne-workshop.github.io/papers/a4ne26-paper6.pdf
...
Go to vercel.com → Sign up with "Continue with GitHub"
Click Add New Project → import your forked repo
Don't deploy yet — you'll add environment variables first (Step 5)
Step 3 — Create a Supabase Project
Go to supabase.com → Sign up with "Continue with GitHub"
Click New Project → name it, set a database password (save it), choose a region → Create
Go to Settings → Data API → copy the Project URL
Go to Settings → API Keys → copy the Publishable key and Secret key
Step 4 — Set Up the Database
In Supabase, go to SQL Editor → New Query
Run the contents of supabase/schema.sql from this repo
Run the contents of supabase/seed_problems.sql to load the 30 default problem statements
Run the contents of supabase/case_files.sql to load the detailed case files used by the AI Simulator Agent
...
APIs & Services → OAuth consent screen → choose External → fill minimal fields → Save
Credentials → Create Credentials → OAuth Client ID → Application type: Web application
Under Authorized redirect URIs, add the callback URL shown on Supabase's Google provider screen (e.g., https://YOUR_PROJECT.supabase.co/auth/v1/callback)
Under Authorized JavaScript origins, add your future Vercel URL (e.g., https://your-app.vercel.app)
Copy the Client ID and Client Secret into Supabase's Google provider fields → Save
...
LLM_KEY_ENCRYPTION_SECRET	Any random 32-character string (generate at randomkeygen.com)
...
Set Site URL to your Vercel URL exactly (e.g., https://your-app.vercel.app)
...
Google Gemini (has a free tier): aistudio.google.com/app/apikey
...
Using the App
First Login
Visit your Vercel URL, sign in with Google
On first login, choose "Become Orchestrator"
Orchestrator Dashboard
Configure LLM Judge — pick a provider, enter the exact model name (see links in-app for current model names, since providers rename these often), enter your API key, click Test Connection, then Save
Select Problem to Assign — choose from the 30 built-in problems (or any you've added — see below)
Add Participants — single entry, bulk paste (Name, Email per line), or upload a .csv/.txt file
Assign Problem & Generate Link — creates a unique link per participant. Copy and share it via any channel (email, Slack, Teams) — the app does not auto-send email
Participant Reports & Scores — view every participant's CSAT average, question credit, turns used, and whether they found the root cause; download any completed report as PDF
Participant Experience
Open the link the orchestrator shared
Sign in with the exact Google account the orchestrator entered — access is granted only if the emails match
Read the (deliberately vague) problem statement
Ask questions or propose actions — check the "This is an Action attempt" box before proposing a fix
An AI Simulator Agent responds in-character based on a detailed hidden case file, and an AI Judge Agent scores each turn on CSAT (0–10) and Question Credit (finite budget)
The exercise ends when: the root cause is correctly identified, Question Credit reaches 0, or the question limit is reached
Download your PDF report — includes your full turn log, final scores, and framework-aligned coaching suggestions
